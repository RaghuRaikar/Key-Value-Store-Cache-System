...
void kvlist_remove_last(kvlist_t *list) {
  // see if list is empty
  if (list->head == NULL) {
    return;
  }

  // prev and node pointers to loop through list
  kvlist_node_t *prev = NULL;
  kvlist_node_t *node = list->head;
  ...
  // look if there is only 1 node in list
  if (prev == NULL) {
    // set both to null
    list->head = NULL;
    list->tail = NULL;
  ...
  // here we free the last node
  kvlist_node_free(&node);
}

void kvlist_move_to_front(kvlist_t *lst, kvpair_t *pair) {
  kvlist_node_t *prev = NULL, *current = lst->head;

  // If the pair is already at the head
  if (lst->head->kv == pair) {
    return;
  }

  // Find the node that needs to be moved
  while (current != NULL && current->kv != pair) {
    prev = current;
    current = current->next;
  }

  // If pair is not found just return
  if (current == NULL) {
    return;
  }
  ...
  // here we put the node at the front
  current->next = lst->head;
  lst->head = current;

  // If the moved node was the tail node
  if (lst->tail == current) {
    lst->tail = prev;
  }
}

void kvlist_insert_at_front(kvlist_t *lst, kvpair_t *pair) {
  // Create a new node with the pair
  kvlist_node_t *front_node = kvlist_node_new(pair);

  // reassign the head
  front_node->next = lst->head;

  // Make the head point to new node
  lst->head = front_node;

  // If the list was empty before adding, update tail
  if (lst->tail == NULL) {
    lst->tail = front_node;
  }
}

kvpair_t *kvlist_last_pair(kvlist_t *list) {
  // create iterator
  kvlist_iterator_t *pair_iterator = kvlist_iterator_new(list);
  // create pair and last pair
  kvpair_t *pair;
  kvpair_t *last_pair = NULL;
  // loop through the list and find last pair
  while ((pair = kvlist_iterator_next(pair_iterator)) != NULL) {
    last_pair = pair;
  }
  // free the iterator
  kvlist_iterator_free(&pair_iterator);
  // either return the last pair otherwise return NULL
  if (last_pair != NULL) {
    return last_pair;
  } else {
    return NULL;
...
char *string_duplicate(const char *string) {
  // make memory to store the duplicate string
  char *character = malloc(strlen(string) + 1);
  if (character == NULL) {
    return NULL;
  }
  // copy the stirng
